# Deploying the LBC Agent to Google Cloud Run
...
gcloud builds submit --tag gcr.io/$PROJECT_ID/printemps-terres-lbc-agent --dockerfile=lbc/Dockerfile
```

Or build locally:

```bash
docker build -f lbc/Dockerfile -t printemps-terres-lbc-agent .
```
...
sed -i "s/PROJECT_ID/$PROJECT_ID/g" lbc/cloudrun-job.yaml

# Deploy the job
gcloud run jobs replace lbc/cloudrun-job.yaml --region=europe-west1
```
...
gcloud run jobs execute printemps-terres-lbc-agent --region=europe-west1

# View logs
gcloud run jobs executions logs printemps-terres-lbc-agent --region=europe-west1
```
...
gcloud scheduler jobs create http printemps-terres-lbc-biweekly \
  --location=europe-west1 \
  --schedule="0 6 * * 1,4" \
  --time-zone="Europe/Paris" \
  --uri="https://europe-west1-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/$PROJECT_ID/jobs/printemps-terres-lbc-agent:run" \
  --http-method=POST \
